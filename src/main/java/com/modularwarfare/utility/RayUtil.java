...
public class RayUtil {
    ...
    public static float calculateAccuracyServer(final ItemGun item, final EntityLivingBase player) {
        ...
        ItemGun sniper = (ItemGun) Minecraft.getMinecraft().player.getItemStackFromSlot(EntityEquipmentSlot.MAINHAND).getItem();

        if (player.posX != player.lastTickPosX || player.posZ != player.lastTickPosZ || player.posY != player.lastTickPosY) {
            acc += 0.2f;
        }
        if (!player.onGround) {
            acc += 0.4f;
        ...
        if (player.isSneaking() && sniper.type.weaponType == WeaponType.BoltSniper || sniper.type.weaponType == WeaponType.Carbine) {
            acc = 0.0f;
        }
    ...
    public static float calculateAccuracyClient(final ItemGun item, final EntityPlayer player) {
        ...
        if (settings.keyBindJump.isKeyDown() || settings.keyBindForward.isKeyDown() || settings.keyBindLeft.isKeyDown() || settings.keyBindBack.isKeyDown() || settings.keyBindRight.isKeyDown()) {
            acc += 0.2f;
        }
        if (!player.onGround) {
            acc += 0.4f;
            ...
            if (!Double.isNaN(vec32.x) && !Double.isNaN(vec32.y) && !Double.isNaN(vec32.z)) {
                ...
                while (k1-- >= 0) {
                    ...
                    if (block1 instanceof BlockGlass || block1 instanceof BlockStainedGlassPane || block1 instanceof BlockStainedGlass) {
                        ...
                        ModularWarfare.NETWORK.sendToAllAround(new PacketPlaySound(blockpos, "impact.glass", 1f, 1f), new NetworkRegistry.TargetPoint(0, blockpos.getX(), blockpos.getY(), blockpos.getZ(), 15));
