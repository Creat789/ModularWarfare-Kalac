...
import com.modularwarfare.common.network.PacketGasClient;
...
public class EntityGasGrenade extends EntityGrenade {
    ...
    public EntityGasGrenade(World world, EntityLivingBase thrower, boolean isRightClick, GrenadeType grenadeType) {
        ...
        this.smokeTime = grenadeType.smokeTime * 30;
        ...
        if (!this.world.isRemote) {
            for (EntityPlayer player : world.playerEntities) {
                    if (player.getDistance(this) < 20) {
                        ModularWarfare.NETWORK.sendTo(new PacketGasClient(90), (EntityPlayerMP) player);
                        player.addPotionEffect(new PotionEffect(Potion.getPotionById(2), 40, 2));
                        player.addPotionEffect(new PotionEffect(Potion.getPotionById(19), 40, 5));
                    }
            }
        }
    }
